...
1. **Incremental Summarization API**: After a text exchange, or when a voice turn completes, the newly-added messages (at least one user+assistant pair) are folded into the session's existing summary via `app/api/summarize/route.ts` — using the same cheap GLM/Grok pipeline used for document refinement, capped under 200 words, and updated incrementally.
2. **Unified Zustand Store Action**: The core asynchronous summarization fetch logic is encapsulated in the global store action `syncTextSummary()` inside `components/paper-talk/chat-store.ts`. This allows any navigation action or hook to trigger synchronization programmatically and track the global `isSyncingSummary` state.
3. **Immediate Switch Trigger**: When transitioning from the Chat view to the Voice view, `app/page.tsx` intercepts the tab transition and immediately invokes `syncTextSummary()`. This runs the summarization fetch immediately during the tab switch, eliminating the 3-second debounce delay.
4. **Lock-and-Retry Queue**: Both the text hook (`useConversationSummarySync`) and the voice panel (`maybeSyncSummary`) utilize a state lock (`isSyncingSummary` in store / `isSyncingSummaryRef` / `inFlightRef`) and a pending sync flag (`pendingSyncRef` / `hasPendingSummarySyncRef`). If another sync is requested while one is in flight, it is queued and executed sequentially when the active fetch resolves, avoiding concurrent summary overwrites.
5. **No Data Loss on Disconnect**: When the voice call is disconnected, `maybeSyncSummary(true)` forces a final sync of any leftover messages (even if it's a single unpaired message) rather than returning early, preserving context.
6. **Visual Sync Loader**: A global `isSyncingSummary` state is tracked in the store. While a sync is in flight, the "Initialize voice call" button is disabled and displays a spinning `Syncing chat context...` loader to prevent the user from connecting with a stale summary.
7. **Synchronous Transcript Updates**: Transcripts in `VoicePanel` update the backing `transcriptsRef.current` synchronously to prevent React state update lag from showing stale data during WebSocket message events.
8. **Context Injection**: The resulting summary is injected into `effectiveInstruction` in `app/page.tsx` alongside identity and document context, reaching both `/api/chat` and the voice WebSocket's `instruction` param — so facts are carried back and forth between Chat and Voice seamlessly.
