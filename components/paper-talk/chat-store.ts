...
const GREETING =
  "Hi, I'm Paper Talk, your personal assistant. Ask me anything, or start a voice call from the left rail.";
...
function placeholderSession(): ChatSession {
  return {
    ...
    title: "New chat",
    messages: [
      { id: "seed-1", role: "assistant", content: GREETING, time: "" },
    ],
    resources: [],
    summary: "",
    ...
    (set, get) => ({
      ...
      addResource: (doc) =>
        set((state) => ({
          sessions: state.sessions.map((session) =>
            session.id === state.activeSessionId
              ? { ...session, resources: [...session.resources, doc] }
              : session,
          ),
        })),
        ...
        set((state) => ({
          sessions: state.sessions.map((session) =>
            session.id === state.activeSessionId
              ? {
                  ...session,
                  resources: session.resources.map((doc) =>
        ...
        set((state) => ({
          sessions: state.sessions.map((session) =>
            session.id === state.activeSessionId
              ? {
                  ...session,
                  resources: session.resources.filter((doc) => doc.id !== id),
                }
              : session,
          ),
        })),
        ...
        set((state) => ({
          sessions: state.sessions.map((session) =>
            session.id === sessionId
              ? { ...session, summary, summarizedThroughCount: throughCount }
              : session,
          ),
        })),
      ...
      ensureInitialized: () => {
        ...
        if (
          sessions.length === 1 &&
          sessions[0].id === PLACEHOLDER_SESSION_ID
        ) {
          const session = createSession();
...
export function useResourceContext() {
  return useChatStore((state) => {
    ...
    return combined.length > MAX_TOTAL_CONTEXT_CHARS
      ? combined.slice(0, MAX_TOTAL_CONTEXT_CHARS)
      : combined;
  });
}
