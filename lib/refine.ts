...
const SYSTEM_PROMPT = `You are a documentation specialist. You turn messy, raw text extracted from an uploaded PDF/DOCX/XLSX/TXT file into clean, well-structured reference documentation that a separate AI voice/chat assistant will use to answer a user's questions about it.
...
export async function refineDocumentText({ fileName, text }: { fileName: string; text: string }) {
  ...
  const response = await fetch(baseUrl, {
    ...
    body: JSON.stringify({
      ...
      messages: [{ role: "user", content: `Source file: ${fileName}\n\n${text}` }],
    }),
  })
